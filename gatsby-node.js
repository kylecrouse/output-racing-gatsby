    ...
    IracingMember: {
      driverMedia: {
        ...
        resolve: async (source, args, context, info) => {
          const { media___NODE = [] } =
            (await getContentfulDriver(source, args, context, info)) ?? {};
      ...
      driverNumberArt: {
        type: "ContentfulAsset",
        resolve: async (source, args, context, info) => {
          const { numberArt___NODE = null } =
            (await getContentfulDriver(source, args, context, info)) ?? {};
          if (!numberArt___NODE) return null;
      ...
      eventBroadcast: {
        ...
        resolve: async (source, args, context, info) => {
          const { broadcast = null } =
            (await getContentfulRace(source, args, context, info)) ?? {};
          return broadcast;
        },
      },
      eventLogo: {
        type: "ContentfulAsset",
        resolve: async (source, args, context, info) => {
          const { logo___NODE = null } =
            (await getContentfulRace(source, args, context, info)) ?? {};
          if (!logo___NODE) return null;
      ...
      eventMedia: {
        ...
        resolve: async (source, args, context, info) => {
          const { media___NODE = [] } =
            (await getContentfulRace(source, args, context, info)) ?? {};
          return Promise.all(
            media___NODE.map((id) => context.nodeModel.getNodeById({ id }))
          );
        },
      },
    },
        ...
        resolve: async (source, args, context, info) => {
          return context.nodeModel
            .findAll({
              query: {
                filter: {
                  driver_id: { eq: source.driver_id },
                  schedule_id: { eq: source.schedule_id },
                },
              },
              type: "MysqlStages",
            })
...
exports.createPages = async ({ graphql, actions }) => {
  ...
  data.series.nodes.forEach(({ seriesId, seriesName, currentSeason }) => {
    ...
    createPage({
      ...
      context: {
        ...
        raceId: currentSeason.schedules.reduce((recent, schedule) => {
          const scheduleDate = new Date(
            `${schedule.raceDate.split("T")[0]}T${schedule.raceTime}`
          );
          const recentDate = recent.raceDate
            ? new Date(`${recent.raceDate.split("T")[0]}T${recent.raceTime}`)
            : null;
          return schedule.race !== null &&
            schedule.chase === "N" &&
            (!recentDate || scheduleDate.getTime() > recentDate?.getTime())
            ? schedule
            : recent;
        }, {}).race?.raceId,
      },
  ...
  data.races.nodes.forEach((node, index) => {
    ...
    const raceId = node.raceId,
      ...
      scheduleId = node.schedule.scheduleId;
