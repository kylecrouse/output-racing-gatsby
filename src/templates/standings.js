...
const StandingsTemplate = (props) => {
  ...
  const rounds = React.useMemo(() => {
    let totalRounds = 0,
      roundsCompleted = 0;
    const rounds = season.schedules.reduce((a, round) => {
        ...
        return {
          label:
            el.chase === "Y"
              ? `-- ${
                  el.eventName ??
                  `Chase: Top ${el.chaseConfig.chaseNumDrivers} Drivers`
                } --`
              : `Round ${raceNo}: ${el.trackConfig?.trackName
                  .replace(/\[.*\]\s/, "")
                  .replace(/ - \d*$/, "")}`,
          chipLabel:
            el.chase === "Y"
  ...
  const priorWeek = React.useMemo(() => {
    ...
    filteredRounds
      ...
      .forEach(({ race }) => {
        race?.participants.forEach((p) => {
          ...
          const totalPoints =
            p.racePoints + stagePoints + bonusPoints - penaltyPoints;
    ...
    return Array.from(participants.entries())
      .map(([driverId, totalPoints]) => ({ driverId, totalPoints }))
    ...
    () => [
      {
        ...
        aggregatedCell: ({ table, row }) => {
          ...
          while (index--) {
            if (
              sortedRows[index]._groupingValuesCache.points >
              row._groupingValuesCache.points
            ) {
              position = index + 1;
              break;
            }
      ...
      },
      {
        ...
        aggregatedCell: ({ table, row }) => {
          const { rows: sortedRows } = table.getSortedRowModel();
          const pos = sortedRows.findIndex(({ id }) => row.id === id),
      ...
      },
      {
        ...
        aggregatedCell: ({ table, row }) => {
          ...
          let points = row._groupingValuesCache.points,
            index = sortedRows.findIndex(({ id }) => row.id === id);
          if (index === 0) return "-";
          else return points - sortedRows[0]._groupingValuesCache.points;
        },
      },
      {
        id: "starts",
      ...
      },
      {
        ...
        header: "Top 5",
        accessorFn: (row) =>
          row.finishPos !== null && row.finishPos <= 5 ? 1 : 0,
        aggregationFn: "sum",
        className: "hide-sm",
      },
      {
        ...
        header: "Top 10",
        accessorFn: (row) =>
          row.finishPos !== null && row.finishPos <= 10 ? 1 : 0,
        aggregationFn: "sum",
        className: "hide-sm",
      },
      {
      ...
      },
      {
        ...
        aggregationFn: (id, rows) => {
          ...
          return (
            rows.reduce((sum, row) => sum + row.getValue(id), 0) / rows.length
          );
        },
        className: "hide-sm",
          ...
          <div className="column col-8 col-xl-12 col-mx-auto content">
            <hgroup className="page-header columns">
              <div className="column col-7 col-lg-12">
                ...
                <h3
                  className="page-subtitle columns"
                  style={{ alignItems: "baseline" }}
                >
                  <div
                    className="col-7 col-lg-12"
                  ...
                  >
                    <Select
                      ...
                      onChange={(selected) => setSeasonId(selected.value)}
...
export const query = graphql`
  query StandingsQuery($seriesId: Int) {
