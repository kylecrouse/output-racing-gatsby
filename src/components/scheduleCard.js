...
const ScheduleCard = (props) => {
  const date = moment(
    `${props.raceDate.split("T")[0]}T${props.raceTime ?? "21:00:00"}`
  ...
  React.useEffect(() => {
    if (!(props.countdown && duration?.asSeconds() > 0)) return;
  ...
  return (
    <div className={`${styles.container} ${props.className}`}>
      <div className={styles.header}>
        {props.seriesId === 11326 ? (
          <>
            <img src={echoLogo} alt="Echo Series" />{" "}
            <img src={echoLogoBG} alt="" className={styles.logoBackground} />
          </>
        ) : props.seriesId === 8100 ? (
      ...
      {props.trackAsset && (
        <div className={styles.map}>
          <img
            src={`${props.trackAsset.map}${props.trackAsset.layers.active}`}
            alt="track map"
          />
        </div>
      )}
      {props.race ? (
        <ResultsChip
          ...
          results={props.race.participants
