...
const IndexPage = (props) => {
  ...
  const { cards = [] } = React.useMemo(
    () =>
      props.data.schedules.nodes.reduce(
        (a, node) => {
          const date = new Date(
            `${node.raceDate.split("T")[0]}T${node.raceTime ?? "20:55:00"}`
          );
          console.log(node);
          if (node.chase === "Y")
  ...
  const series = React.useMemo(
    () =>
      props.data.series.nodes.reduce(
        (a, node) => ({
          ...a,
          [node.seriesId]: (
            <section
              ...
              <div className={styles.sectionContainer}>
                <div className={styles.sectionScreenshot}>
                  <div
                    style={{
                      backgroundImage: `url(${
                        node.seriesId === 8100 ? nightOwlImage : outputImage
                      })`,
                    }}
                  />
                </div>
                <div className={styles.sectionContent}>
                  ...
                  {node.currentSeason?.schedules.some(
                    ({ race }) => race !== null
                  ) > 0 && (
                    <>
                      ...
                      {Array.from(
                        node.currentSeason?.schedules.reduce(
                          (acc, schedule) => {
                            schedule.race?.participants.forEach((p) => {
                              const bonusPoints =
                                p.bonuses?.reduce(
                                  (points, { bonusPoints }) =>
                                    (points += bonusPoints),
                              ...
                              const key =
                                p.driver.member?.driverNickName ??
                                p.driver.driverName;
                        ...
                        .sort((a, b) => b[1] - a[1])
                        .slice(0, 3)
                        .map(([driverName, totalPoints], index) => (
                          <StandingsCard
                            key={`standings-${index}`}
                            position={index + 1}
                            driver={{ driverName }}
...
export const query = graphql`
  query IndexQuery {
    ...
    schedules: allMysqlSchedule(sort: { race_date: ASC }) {
      nodes {
